...
let lbl' = (List.map (fun (x,y) -> if ((Float.add x y) < 1.1) then 1. else -1. ) alc) in 
...
let pred = (List.map (fun (x,y) -> if (Float.add (Float.add (Float.mul x1 x) (Float.mul y1 y)) 1.0 > 0.) then 1. else -1. ) alc) in
let corr = List.fold_left2 (fun acc x y -> if (x=y) then acc + 1 else acc) 0 pred lbl' in 
